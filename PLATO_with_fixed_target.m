...
% Shortest path vector: V_path = target - start
V_path_shortest = target - start;
...
deceleration_factor = 2; % Tuning factor: >1 starts deceleration earlier, <1 later
...
% State: [x; y; s; psi; integral_heading_error; previous_heading_error; previous_speed_error]
...
% Running simulations for each guidance algorithm
...
calculate_rdot = @(Y, target_pos, current_vec) ...
-( ...
((target_pos(1) - Y(:,1)) .* (Y(:,3) .* cos(Y(:,4)) + current_vec(1))) + ...
((target_pos(2) - Y(:,2)) .* (Y(:,3) .* sin(Y(:,4)) + current_vec(2))) ...
) ./ sqrt((target_pos(1) - Y(:,1)).^2 + (target_pos(2) - Y(:,2)).^2);

% rdot for each guidance law
rdot_LOS = calculate_rdot(Y_LOS, target, current_vector');
rdot_carrot = calculate_rdot(Y_carrot, target, current_vector');
rdot_NLG = calculate_rdot(Y_NLG, target, current_vector');
...
% Non-Linear Guidance
rc = nonlinear_guidance(x, y_pos, psi, params, v_auv_ground, s_ground, psi_ground);
heading_error = 0;
d_integral_heading_error = 0;
else
% Standard PID controller for LOS and Carrot Chase
if strcmp(params.guidance, 'carrot')
desired_psi_ground = carrot_guidance(x, y_pos, params);
else % Default to LOS
...
% Dynamic deceleration radius based on current ground speed, max deceleration and docking radius.
...
if dist_to_target > 0
v_docking_des = params.docking_speed * ([target(1); target(2)] - [x; y_pos]) / dist_to_target;
else
v_docking_des = [0;0];
end
...
(log(dynamic_decel_radius) - log(params.docking_radius));
...
% Apply PID control law for acceleration/deceleration
...
function rc = nonlinear_guidance(x, y_pos, psi, params, v_auv_ground, s_ground, psi_ground)
start = params.start;
...
current_vector = params_carrot.current_vector;
...
% Get ground-relative velocities for plotting
...
% Get dynamically calculated deceleration radius for plotting
deceleration_distance_at_cruise = params_carrot.deceleration_factor * (params_carrot.cruise_speed^2 / (2 * params_carrot.max_deceleration));
dynamic_decel_radius_at_cruise = params_carrot.docking_radius + deceleration_distance_at_cruise;
...
% 2D side view frustum aligned with the shortest path
radius1 = 1.0; % Bigger radius (m)
radius2 = 0.5; % Smaller radius (m)
length_frustum = 3.0; % Length of the frustum (m)
% Vector from target to start
path_vec = start - target;
path_length = norm(path_vec);
if path_length > 1e-6
path_unit = path_vec / path_length;
% Centers of the frustum's circles
center2 = target; % Smaller circle center is at the target
center1 = target + length_frustum * path_unit; % Bigger circle center is along the path
% Points for the frustum's side view
% The frustum is a trapezoid in 2D
normal_unit = [-path_unit(2); path_unit(1)]; % Normal vector to the path
p1 = center1 + radius1 * normal_unit;
p2 = center1 - radius1 * normal_unit;
p3 = center2 - radius2 * normal_unit;
p4 = center2 + radius2 * normal_unit;
...
plot([center1(1), center2(1)], [center1(2), center2(2)], 'k-.', 'LineWidth', 1.0, 'DisplayName', 'Frustum Centerline');
end

% Plot the shortest path line segment
plot([start(1), target(1)], [start(2), target(2)], 'k--', 'LineWidth', 1.0, 'DisplayName', 'Desired Path');
hold off;
axis equal; grid on;
xlabel('X (m)'); ylabel('Y (m)');
title('AUV Trajectory Comparison with Current');
...
yline(norm(current_vector) + params_carrot.docking_speed, 'r--', 'Desired Final Speed');
