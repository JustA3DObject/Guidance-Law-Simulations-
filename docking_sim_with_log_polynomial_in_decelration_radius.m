...
common_params = struct('Kp_heading', Kp_heading, 'Ki_heading', Ki_heading, 'Kd_heading', Kd_heading, ...
                       'Kp_speed', Kp_speed, 'Ki_speed', Ki_speed, 'Kd_speed', Kd_speed, ...
...
rdot_LOS = calculate_rdot(Y_LOS, target);
rdot_carrot = calculate_rdot(Y_carrot, target);
rdot_NLG = calculate_rdot(Y_NLG, target);
rdot_TPN = calculate_rdot(Y_TPN, target);
rdot_RTPN = calculate_rdot(Y_RTPN, target);
...
% AUV Model with Dynamic Deceleration Control
...
function dydt = auv_model_log_poly_decel(t, y, params)
    ...
    integral_speed_error = y(7); % NEW: Integral of speed error
    previous_speed_error = y(8); % NEW: Previous speed error
    
    % Initialize speed_error and ds to default values
    speed_error = 0;
    ds = 0;
    ...
    elseif strcmp(params.guidance, 'TPN') || strcmp(params.guidance, 'RTPN')
        ...
        rc = params.Kp_heading * heading_error + params.Ki_heading * integral_heading_error + params.Kd_heading * derivative_heading_error;
    end
    
    % Speed Control - Dynamic Deceleration Logic
    dist_to_target = sqrt((x - target(1))^2 + (y_pos - target(2))^2);
    
    % Calculate the dynamic deceleration radius based on current speed, max deceleration and docking radius.
    ...
    elseif dist_to_target > dynamic_decel_radius
        ...
        if dist_to_target <= params.docking_radius
            ...
            n_min = 2;
            n_max = 5;
            % Normalize current speed relative to cruise speed
        ...
        % PID control to achieve desired speed
        speed_error = s_desired - s;
        d_integral_speed_error = speed_error;
        derivative_speed_error = speed_error - previous_speed_error;
        
        ds = params.Kp_speed * speed_error + ...
             params.Ki_speed * integral_speed_error + ...
             params.Kd_speed * derivative_speed_error;
    end
...
function [rc, heading_error] = tpn_rtpn_guidance(x, y_pos, s, psi, int_e_psi, params)
    ...
    % FIX: Add a small numerical jitter if theta_dot is perfectly or near-perfectly zero.
    if abs(theta_dot) < 1e-12 
       theta_dot = sign(theta_dot) * 1e-12; % Preserves the sign and provides a tiny nudge
    end
    ...
    pid_command = params.Kp_heading * heading_error + params.Ki_heading * int_e_psi + params.Kd_heading * derivative_term;
...
% Extract parameters
    ...
    % This is an approximation as it changes throughout the simulation
    ...
    plot(target(1) + dynamic_decel_radius_at_cruise * cos(theta_circle), target(2) + dynamic_decel_radius_at_cruise * sin(theta_circle), '-.', 'Color', '#999900', 'LineWidth', 1.0, 'DisplayName', 'Decel. Zone (Approx)');
    hold off;
    axis equal; grid on;
